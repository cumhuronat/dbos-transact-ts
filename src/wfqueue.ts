...
interface QueueDispatchRun {
  readonly abortController: AbortController;
  readonly inFlightPolls: Map<string, Promise<void>>;
  isRunning: boolean;
...
class WFQueueRunner {
  ...
  private activeRun?: QueueDispatchRun;
   ...
   */
  async quiesceRootWorkflows(): Promise<void> {
    const run = this.activeRun;
    if (!run?.isRunning) {
  ...
  private async pollQueue(exec: DBOSExecutor, queue: WorkflowQueue, run: QueueDispatchRun): Promise<boolean> {
