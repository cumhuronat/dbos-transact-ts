"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.stopDockerPg = exports.startDockerPg = void 0;
const pg_1 = require("pg");
const utils_1 = require("../utils");
const util_1 = require("util");
const child_process_1 = require("child_process");
/**
 * Starts a PostgreSQL database in a Docker container.
 *
 * This function checks if Docker is installed, and if so, starts a local PostgreSQL database in a Docker container.
 * It configures the database with default settings and provides connection information upon successful startup.
 *
 * The function uses environment variable PGPASSWORD if available, otherwise  defaults to 'dbos' as the database password.
 *
 * @returns null
 *
 * @throws {Error} If there is an error starting the Docker container or if the PostgreSQL service does not become available within the timeout period
 */
async function startDockerPg() {
    console.log('Attempting to create a Docker Postgres container...');
    const hasDocker = await checkDockerInstalled();
    const poolConfig = {
        host: 'localhost',
        port: 5432,
        password: process.env.PGPASSWORD || 'dbos',
        user: 'postgres',
        database: 'postgres',
        connectionTimeoutMillis: 2000,
    };
    if (hasDocker) {
        await startDockerPostgres(poolConfig);
        console.log(`Postgres available at postgres://postgres:${poolConfig.password}@${poolConfig.host}:${poolConfig.port}`);
    }
    else {
        console.warn('Docker not detected locally');
    }
}
exports.startDockerPg = startDockerPg;
async function checkDbConnectivity(config) {
    const pool = new pg_1.Pool(config);
    try {
        const client = await pool.connect();
        try {
            await client.query('SELECT 1;');
            return null;
        }
        catch (error) {
            return error instanceof Error ? error : new Error('Unknown database error');
        }
        finally {
            client.release();
        }
    }
    catch (error) {
        return error instanceof Error ? error : new Error('Failed to connect to database');
    }
    finally {
        // End the pool
        await pool.end();
    }
}
const execAsync = (0, util_1.promisify)(child_process_1.exec);
async function startDockerPostgres(poolConfig) {
    console.log('Starting a Postgres Docker container...');
    const containerName = 'dbos-db';
    const pgData = '/var/lib/postgresql/data';
    // Create and start the container
    const dockerCmd = `docker run -d \
            --name ${containerName} \
            -e POSTGRES_PASSWORD=${poolConfig.password} \
            -e PGDATA=${pgData} \
            -p ${poolConfig.port}:5432 \
            -v ${pgData}:${pgData}:rw \
            --rm \
            pgvector/pgvector:pg16`;
    await execAsync(dockerCmd);
    // Wait for PostgreSQL to be ready
    let attempts = 30;
    while (attempts > 0) {
        if (attempts % 5 === 0) {
            console.log('Waiting for Postgres Docker container to start...');
        }
        if ((await checkDbConnectivity(poolConfig)) === null) {
            return true;
        }
        attempts--;
        await (0, utils_1.sleepms)(1000);
    }
    throw new Error(`Failed to start Docker container: Container ${containerName} did not start in time.`);
}
async function checkDockerInstalled() {
    try {
        await execAsync('docker --version');
        return true;
    }
    catch (error) {
        return false;
    }
}
/**
 * Stops the Docker Postgres container.
 *
 * @returns {Promise<boolean>} True if the container was successfully stopped, false if it wasn't running
 * @throws {Error} If there was an error stopping the container
 */
async function stopDockerPg() {
    const containerName = 'dbos-db';
    try {
        console.log(`Stopping Docker Postgres container ${containerName}...`);
        // Check if container exists and is running
        const { stdout: containerStatus } = await execAsync(`docker ps -a -f name=${containerName} --format "{{.Status}}"`);
        if (!containerStatus) {
            console.log(`Container ${containerName} does not exist.`);
            return false;
        }
        const isRunning = containerStatus.toLowerCase().includes('up');
        if (!isRunning) {
            console.log(`Container ${containerName} exists but is not running.`);
            return false;
        }
        // Stop the container
        await execAsync(`docker stop ${containerName}`);
        console.log(`Successfully stopped Docker Postgres container ${containerName}.`);
        return true;
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        console.error(`Failed to stop Docker Postgres container: ${errorMessage}`);
        throw error;
    }
}
exports.stopDockerPg = stopDockerPg;
//# sourceMappingURL=docker_pg_helper.js.map